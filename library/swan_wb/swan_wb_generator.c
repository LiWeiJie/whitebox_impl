...
static unsigned char S[16] = {0x07, 0x02, 0x0d, 0x04, 0x0c, 0x01, 0x00, 0x09, 0x08, 0x05, 0x0e, 0x0f, 0x06, 0x03, 0x0a, 0x0b};

static unsigned char double_S[256] = {
    
};
...
int _swan_whitebox_helper_init(swan_whitebox_helper *swh)
{
    ...
    for (i=0; i < swh->rounds; i++) {
        for (j=0; j < swh->aff_in_round; j++) {
            ...
            // ind_combined_affine_init(ptr++, SWAN_PIECE_BIT, swh->piece_count);
        }
    }
    combined_affine_init(&(swh->se[0]), SWAN_PIECE_BIT, swh->piece_count);
    combined_affine_init(&(swh->se[1]), SWAN_PIECE_BIT, swh->piece_count);
    // ind_combined_affine_init(&(swh->se[0]), SWAN_PIECE_BIT, swh->piece_count);
    // ind_combined_affine_init(&(swh->se[1]), SWAN_PIECE_BIT, swh->piece_count);
...
// TODO:默认为64bit的秘钥长度
{
    ...
    for (i=0; i< round; i++) {
        memcpy(ptr, key + (i%2?4:0), t);
        ptr += t;
    }
    // print_u8(swh->key_schedule, 4*swh->rounds);
    return 0;
}
...
int _swan_whitebox_content_assemble(swan_whitebox_helper* swh, swan_whitebox_content* swc)
{
    ...
    for (round_id=1; round_id<swc->rounds; round_id++) {
        ...
        // *****************************
        uint8_t * round_key_ptr = swh->key_schedule + swh->block_size/16*round_id;
        // printf("Round 0 key xor: \t");
